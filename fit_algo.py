...
def best_fit_nodes(flow, available_graph):
    ...
    for function in flow.nodes():
        if function in placed_funcs.keys():
            ...
            placed_nodes.append(
                (function, {'cpu': cpu, 'func': flow.nodes[function]['func'], 'place': place}))
        else:
            cpu = flow.nodes[function]['cpu']
            ...
            available_graph.add_node(
                place, cpu=available_graph.nodes[place]['cpu']-cpu, functions=functions)
            graph_bwd.add_node(
                place, cpu=graph_bwd.nodes[place]['cpu']-cpu, functions=functions)
...
def worst_fit_path(placed_flow, available_graph):
    ...
    for k in range(len(func_list)-1):
        if placed_flow.nodes[func_list[k]]['place'] == placed_flow.nodes[func_list[k+1]]['place']:
            path_servers.append(
                (placed_flow.nodes[func_list[k+1]]['place'], func_list[k+1]))
        else:
            chemin = paths_to_function(
                func_list[k], func_list[k+1], bwd, placed_flow, graph_bwd, available_graph)
            path_servers += chemin
...
def paths_to_function(start_func, end_func, bwd, placed_flow, graph_bwd, available_graph):
    ...
    nodes_by_step = [{placed_flow.nodes[start_func]['place']: {
        'parent': None, 'nb_edges': 0, 'min_bwd': math.inf, 'function': start_func}}]
    ...
    while end_server not in nodes_by_step[-1].keys():
        ...
        for server in nodes_by_step[-1].keys():
            ...
            for adj_node in neighbors:
                min_bwd = min(nodes_by_step[-1][server]['min_bwd'],
                              graph_bwd.edges[server, adj_node]['bandwidth'] - bwd)
                ...
                if adj_node not in accessible_servers.keys():
                    if adj_node not in nodes_by_step[-1].keys():
                        accessible_servers[adj_node] = {
                            'parent': server, 'nb_edges': nodes_by_step[-1][server]['nb_edges']+1, 'min_bwd': min_bwd, 'function': funct}
    ...
    if end_server in nodes_by_step[-1].keys():
        chemin = get_path(list(reversed(nodes_by_step)),
                          end_server, bwd, available_graph)
    else:
        chemin = get_path(nodes_by_step[:(
            len(nodes_by_step)-1)].reverse(), end_server, bwd, available_graph)
    return chemin
